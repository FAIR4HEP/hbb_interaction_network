...
def make_plots(outputDir, dataframes, tdf_train, savedirs=["Plots"], taggerNames=["IN"], eraText=r"2016 (13 TeV)"):
    ...
    def plot_rocs(dfs=[], savedir="", names=[], sigs=[["Hcc"]], bkgs=[["Hbb"]], norm=False, plotname=""):
        ...
        if len(sig) == 1 and len(sig[0]) == 3 and sig[0][0] in ["H", "Z", "g"]:
            ...
            xlab = [
                "{} \\rightarrow {}".format(l[0], l[-2] + "\\bar{" + l[-1] + "}") if l[0][0] in ["H", "Z", "g"] else l
                for l in sig
            ]
        ...
        leg = ax.legend(
            ...
            fontsize=16,
            title=""
            + str(int(round((min(frame.fj_pt)))))
            + " $\mathrm{<\ jet\ p_T\ <}$ "
            + str(int(round((max(frame.fj_pt)))))
            + " GeV"
            + "\n "
            + str(int(round((min(frame.fj_sdmass)))))
            + " $\mathrm{<\ jet\ m_{SD}\ <}$ "
            + str(int(round((max(frame.fj_sdmass)))))
            + " GeV",
        )
        ...
        ax.annotate(
            ...
            fontname="Helvetica",
            ha="left",
            bbox={"facecolor": "white", "edgecolor": "white", "alpha": 0, "pad": 13},
        ...
        ax.annotate(
            "$Simulation\ Open\ Data$", xy=(0.115, 1.1), fontsize=18, fontstyle="italic", ha="left", annotation_clip=False
        )
        if norm:
    ...
    def plot_rocs_with_DDT(
        dfs=[], savedir="", names=[], sigs=[["Hcc"]], bkgs=[["Hbb"]], norm=False, plotname="", DDT_results=[[], []]
    ):
        f, ax = plt.subplots(figsize=(10, 10))
        colors = [
            "C0",
        ...
        for frame, name, sig, bkg, color, style, width in zip(dfs, names, sigs, bkgs, colors, line_styles, line_widths):
            ...
            if color not in ["C6", "C1", "C2"]:
                ...
                ax.plot(
                    ...
                    color=color,
                    linestyle=style,
                    label="{}, AUC = {:.1f}\%".format(name, auc(fpr, tpr) * 100),
                )
                ROCtext = open(
                    os.path.join(savedir, "ROCComparison_" + "+".join(sig) + "_vs_" + "+".join(bkg) + ".txt"), "w"
                )
                for ind in range(len(tpr)):
                    ROCtext.write(str(tpr[ind]) + "\t" + str(fpr[ind]) + "\n")
                ROCtext.close()
        ...
        ax.plot(
            ...
            color="C3",
            linestyle="-",
            label="{}, AUC = {:.1f}\%".format("Interaction network, DDT", auc(DDT_results[1], DDT_results[0][0]) * 100),
        )
        ...
        for frame, name, sig, bkg, color, style, width in zip(dfs, names, sigs, bkgs, colors, line_styles, line_widths):
            ...
            if color in ["C2"]:
                ...
                ax.plot(
                    ...
                    color="C5",
                    linestyle=":",
                    label="{}, AUC = {:.1f}\%".format("Deep double-b+, DDT", auc(DDT_results[1], DDT_results[0][3]) * 100),
                )
            ...
            if color in ["C6"]:
                ...
                ax.plot(
                    ...
                    linestyle="--",
                    label="{}, AUC = {:.1f}\%".format(
                        "All-particle interaction network, DDT", auc(DDT_results[1], DDT_results[0][1]) * 100
                    ),
                )
            ...
            if color in ["C1"]:
                ...
                ax.plot(
                    ...
                    color="C4",
                    linestyle="-.",
                    label="{}, AUC = {:.1f}\%".format("Deep double-b, DDT", auc(DDT_results[1], DDT_results[0][2]) * 100),
                )
        ...
        if norm:
            f.savefig(
                os.path.join(savedir, "ROCNormComparison_withDDT_" + "+".join(sig) + "_vs_" + "+".join(bkg) + ".pdf"),
                dpi=400,
            )
            f.savefig(
                os.path.join(savedir, "ROCNormComparison_withDDT_" + "+".join(sig) + "_vs_" + "+".join(bkg) + ".png"),
                dpi=400,
            )
        else:
            f.savefig(
                os.path.join(savedir, "ROCComparison_withDDT_" + "+".join(sig) + "_vs_" + "+".join(bkg) + ".pdf"), dpi=400
            )
            f.savefig(
                os.path.join(savedir, "ROCComparison_withDDT_" + "+".join(sig) + "_vs_" + "+".join(bkg) + ".png"), dpi=400
            )
        plt.close(f)
    ...
    def quantile_regression_DDT_FPR(dfs, dfs_train, FPR_cut):
        ...
        data_train = [
            [mass]
            for mass, pT in zip(
                dfs.loc[dfs["truth" + "QCD"] == 1]["fj_sdmass"].values, dfs.loc[dfs["truth" + "QCD"] == 1]["fj_pt"].values
            )
        ]
        data = [[mass] for mass, pT in zip(dfs["fj_sdmass"].values, dfs["fj_pt"].values)]

        if FPR_cut > 10:
            aux_scale = 0.1 * float(FPR_cut) / 3000.0

        else:
            aux_scale = 0.1 * float(FPR_cut) / 100.0

        model = GradientBoostingRegressor(
            loss="quantile",
            alpha=1 - float(FPR_cut) / 100,
            n_estimators=500,
            min_samples_leaf=50,
    ...
    def quantile_regression_DDT_TPR(dfs, dfs_train, TPR_cut):
        ...
        data_train = [
            [mass]
            for mass, pT in zip(
                dfs.loc[dfs["truth" + "Hbb"] == 1]["fj_sdmass"].values, dfs.loc[dfs["truth" + "Hbb"] == 1]["fj_pt"].values
            )
        ]
        data = [[mass] for mass, pT in zip(dfs["fj_sdmass"].values, dfs["fj_pt"].values)]

        if TPR_cut > 10:
            aux_scale = 0.1 * float(TPR_cut) / 3000.0

        else:
            aux_scale = 0.1 * float(TPR_cut) / 100.0

        model = GradientBoostingRegressor(
            loss="quantile",
            alpha=float(TPR_cut) / 100,
            n_estimators=500,
            min_samples_leaf=50,
            min_samples_split=2500,
            max_depth=5,
    ...
    def plot_jsd(dfs=[], savedir="", names=[], sigs=[["Hcc"]], bkgs=[["Hbb"]], norm=False, plotname=""):
        ...
        for frame, name, sig, bkg, col, style in zip(dfs, temp_names, sigs, bkgs, colors, line_styles):
            ...
            if name in ["Interaction network", "Deep double-b", "Deep double-b+"]:
                ...
                for wp, marker in zip([0.5, 0.9, 0.95], ["^", "s", "o", "v"]):  # % signal eff
                    ...
                    print(
                        "eS = %.2f%%, eB = %.2f%%, 1/eB=%.2f, jsd = %.2f, 1/jsd = %.2f"
                        % (tpr[idx] * 100, fpr[idx] * 100, 1 / fpr[idx], jsd, 1 / jsd)
                    )
                ...
                for wp, marker in zip([50.0, 90.0, 95.0], ["^", "s", "o", "v"]):  # % signal eff.
                    ...
                    FPR_wp = 1.0 / make_FPR_DDT(
                        cut(frame), 1 + wp, siglab="QCD", sculp_label="fj_sdmass", savedir=savedir, taggerName=name
                    )
                    print(
                        "eS = %.2f%%, eB = %.2f%%, 1/eB=%.2f, jsd = %.2f, 1/jsd = %.2f"
                        % (tpr[idx] * 100, fpr[idx] * 100, 1 / fpr[idx], jsd, 1 / jsd)
        ...
        circle = mlines.Line2D(
            [], [], color="gray", marker="o", linestyle="None", markersize=12, label=r"$\varepsilon_\mathrm{sig}$ = 95\%%"
        )
        square = mlines.Line2D(
            [], [], color="gray", marker="s", linestyle="None", markersize=12, label=r"$\varepsilon_\mathrm{sig}$ = 90\%%"
        )
        utriangle = mlines.Line2D(
            [], [], color="gray", marker="^", linestyle="None", markersize=12, label=r"$\varepsilon_\mathrm{sig}$ = 50\%%"
        )
        ...
        ax.annotate(
            "$Simulation\ Open\ Data$", xy=(2.3, 1.1e9), fontsize=18, fontstyle="italic", ha="left", annotation_clip=False
        )
    ...
    def plot_jsd_sig(dfs=[], savedir="", names=[], sigs=[["Hcc"]], bkgs=[["Hbb"]], norm=False, plotname=""):
            ...
            if name not in ["Interaction network, DDT"]:
                ...
                for wp, marker in zip([0.01, 0.05, 0.1], ["^", "s", "o"]):  # % bkg rej.
                    ...
                    for i in range(100):
                        ...
                        mask_pass = (frame.iloc[index]["predict" + sig[0]] > cuts[str(wp)]) & frame.iloc[index][
                            "truth" + bkg[0]
                        ]
                        mask_fail = (frame.iloc[index]["predict" + sig[0]] < cuts[str(wp)]) & frame.iloc[index][
                            "truth" + bkg[0]
                        ]
                ...
                for wp, marker in zip([1.0, 5.0, 10.0], ["^", "s", "o"]):  # % bkg rej.
                    ...
                    jsd_plot.append(1 / jsd)
                    ax.plot(
                        [make_TPR_DDT(frame, wp, siglab="QCD", sculp_label="fj_sdmass", taggerName=name)],
                        [1 / jsd],
                        marker=marker,
                        markersize=12,
                        color=color,
        ...
        ax.set_ylim(1, 5e4)
        ax.set_xlabel(
            r"Tagging efficiency ($\mathrm{H \rightarrow b\bar{b}}$) $\varepsilon_\mathrm{sig}$", ha="right", x=1.0
        )
        ax.set_ylabel(r"Mass decorrelation 1 / $D_\mathrm{JS}$", ha="right", y=1.0)
        ...
        circle = mlines.Line2D(
            [], [], color="gray", marker="o", linestyle="None", markersize=12, label=r"$\varepsilon_\mathrm{bkg}$ = 10\%%"
        )
        square = mlines.Line2D(
            [], [], color="gray", marker="s", linestyle="None", markersize=12, label=r"$\varepsilon_\mathrm{bkg}$ = 5\%%"
        )
        utriangle = mlines.Line2D(
            [], [], color="gray", marker="^", linestyle="None", markersize=12, label=r"$\varepsilon_\mathrm{bkg}$ = 1\%%"
        )
        # dtriangle = mlines.Line2D([], [], color='gray', marker='v', linestyle='None',
        ...
        ax.annotate(
            "$Simulation\ Open\ Data$", xy=(0.105, 5.1e4), fontsize=18, fontstyle="italic", ha="left", annotation_clip=False
        )
    ...
    def DDT_Accuracy(
        tdf, TPR_cut=50, siglab="Hbb", sculp_label="Light", sig=["Hbb"], bkg=["QCD"], savedir="", taggerName=""
    ):
    ...
    def make_DDT(tdf, FPR_cut=[], siglab="Hcc", sculp_label="Light", savedir="", taggerName=""):
        ...
        for FPR in range(len(FPR_cut)):
            ...
            ax.hist(
                dataframes_cut[FPR]["fj_sdmass"].values,
                bins=bins,
            ...
            n, binEdges = np.histogram(
                dataframes_cut[FPR].loc[dataframes_cut[FPR]["truth" + "QCD"] == 1]["fj_sdmass"].values, bins=bins
            )
        ...
        ax.hist(
            tdf["fj_sdmass"].values,
            bins=bins,
            weights=weight_uncut / np.sum(weight_uncut),
            lw=2,
            normed=False,
            histtype="step",
            label="No tagging applied",
        )
    ...
    def sculpting_multiple_taggers(tdf_array, names_array, siglab="Hbb", sculp_label="QCD", savedir="", sculpt_wp=0.01):
        ...
        for tdf, name, style, width, col in zip(tdf_array, names_array, line_styles, line_widths, colors):
            ...
            if name in ["Interaction network", "Deep double-b", "Deep double-b+"]:
                ...
                cuts = {}
                for wp in [1.0, sculpt_wp]:  # % mistag rate
                    idx, val = find_nearest(fpr, wp)
                    cuts[str(wp)] = threshold[idx]  # threshold for deep double-b corresponding to ~1% mistag rate
    ...
    def sculpting(tdf, siglab="Hcc", sculp_label="Light", savedir="", taggerName=""):
        ...
        for wp, cut in reversed(sorted(cuts.items())):
            ...
            if str(wp) == "1.0":
                ...
                ax.hist(
                    ...
                    normed=False,
                    histtype="step",
                    label="{}\%  mistagging rate".format(float(wp) * 100.0),
                )
        ...
        leg = ax.legend(
            ...
            fontsize=16,
            title=""
            + str(int(round((min(tdf.fj_rho)))))
            + " $\mathrm{< rho <}$ "
            + str(int(round((max(tdf.fj_rho)))))
            + " GeV"
            + "\n {} tagging {}".format(taggerName, legend_siglab),
        )
        leg._legend_box.align = "right"
    ...
    def eta_dep(xdf, sig_label="Hcc", bkg_label="", savedir="", taggerName=""):
        ...
        ylab1 = [
            "{} \\rightarrow {}".format(l[0], l[-2] + "\\bar{" + l[-1] + "}")
            if len(l) == 3 and l[0] in ["H", "Z", "g"]
            else l
            for l in [sig_label]
        ][0]
    ...
    def multiple_pu_dep(tdf_array, names_array, sig_label="Hcc", bkg_label="", savedir="", sculpt_wp=0.01):
        ...
        for xdf, taggerName, style, width, col in zip(tdf_array, names_array, line_styles, line_widths, colors):
            ...
            if taggerName in ["Interaction network", "Deep double-b", "Deep double-b+"]:
                ...
                for pu in pts:
                    ...
                    efftight.append(
                        make_TPR_DDT(
                            cut_pu(xdf, pu, pu + step),
                            1.0,
                            siglab="QCD",
                            sculp_label="fj_sdmass",
                            savedir=savedir,
        ...
        mpt = (
            ""
            + str(int(round((min(xdf.fj_pt)))))
            + " $\mathrm{<\ jet\ p_T\ <}$ "
            + str(int(round((max(xdf.fj_pt)))))
            + " GeV"
            + "\n "
            + str(int(round((min(xdf.fj_sdmass)))))
            + " $\mathrm{<\ jet\ m_{SD}\ <}$ "
            + str(int(round((max(xdf.fj_sdmass)))))
            + " GeV"
    ...
    def overlay_distribution(xdf, savedir="", feature="pt", truths=["Hcc", "QCD"], app_weight=True):
        ...
        # Set Alpha
        if np.sum([1 for tru in truths if len(tru) > 1]) > 4:
            a = 0.5
        elif np.sum([1 for tru in truths if len(tru) > 1]) > 2:
            a = 0.7
        else:
            a = 0.8
            ...
            if app_weight:
                if tru == "":
                    ax.hist(
                        pred,
                        bins=bins,
                        weights=weight * xdf.Weight / np.sum(weight * xdf.Weight),
                        alpha=a,
                        normed=False,
                        label="",
                    )  # Mock plot
                ...
                if tru == "":
                    ax.hist(pred, bins=bins, weights=weight / np.sum(weight), alpha=a, normed=False, label="")  # Mock plot
                else:
                    ax.hist(
                        pred,
                        bins=bins,
        ...
        if feature == "m":
            ...
            try:
                ax.set_xlabel(
                    r"Dicriminator $\mathrm{}$".format("{" + legend_labels[all_labels.index(feature)] + "}"),
                    ha="right",
                    x=1.0,
                )
            except:
        ...
        if app_weight:
            f.savefig(
                os.path.join(savedir, "dist_weight_" + feature + "_" + "-".join(t for t in truths if len(t) > 1) + ".pdf"),
                dpi=400,
            )
            f.savefig(
                os.path.join(savedir, "dist_weight_" + feature + "_" + "-".join(t for t in truths if len(t) > 1) + ".png"),
                dpi=400,
            )
        else:
            f.savefig(
                os.path.join(savedir, "dist_" + feature + "_" + "-".join(t for t in truths if len(t) > 1) + ".pdf"), dpi=400
            )
            f.savefig(
                os.path.join(savedir, "dist_" + feature + "_" + "-".join(t for t in truths if len(t) > 1) + ".png"), dpi=400
            )
        plt.close(f)
    ...
    plot_rocs(
        dfs=[cut(frame) for frame in dataframes],
    ...
    plot_rocs_with_DDT(
        dfs=[cut(cutrho(frame)) for frame in dataframes],
        savedir=os.path.join(outputDir, "Plots"),
        names=taggerNames,
        sigs=[["Hbb"], ["Hbb"], ["Hbb"], ["Hbb"], ["Hbb"]],
        bkgs=[["QCD"], ["QCD"], ["QCD"], ["QCD"], ["QCD"]],
        DDT_results=[TPR_range, FPR_range],
    )
    ...
    sculpting_multiple_taggers(
        [cut(cutrho(frame)) for frame in dataframes],
        taggerNames,
        siglab="Hbb",
        sculp_label="QCD",
        savedir=os.path.join(outputDir, "Plots"),
    ...
    for frame, savedir, taggerName in zip(dataframes, savedirs, taggerNames):
        ...
        for label in labels:
            for label2 in labels:
                if label == label2:
                    continue
                plot_rocs(dfs=[frame], savedir=savedir, names=[taggerName], sigs=[[label]], bkgs=[[label2]])
            ...
            plot_rocs(
                dfs=[frame], savedir=savedir, names=[taggerName], sigs=[[label]], bkgs=[[l for l in labels if l != label]]
            )
