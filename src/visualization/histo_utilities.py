...
def create_TH1D(
    x, name="h", title=None, binning=[None, None, None], weights=None, h2clone=None, axis_title=["", ""], opt="", color=0
):
    if title is None:
        title = name
    if x.shape[0] == 0:
...
def create_prof1D(
    x, y, name="h", title=None, binning=[None, None, None], h2clone=None, axis_title=["", ""], opt="", color=0
):
    if title is None:
        title = name
    if h2clone == None:
...
def create_TH2D(
    sample, name="h", title=None, binning=[None, None, None, None, None, None], weights=None, axis_title=["", "", ""]
):
    if title is None:
        title = name
    if sample.shape[0] == 0:
        ...
        if binning[0] is None:
            bin_w = (
                4 * (np.percentile(sample[:, 0], 75) - np.percentile(sample[:, 0], 25)) / (len(sample[:, 0])) ** (1.0 / 3.0)
            )
            if bin_w == 0:
        ...
        if binning[3] == None:
            bin_w = (
                4 * (np.percentile(sample[:, 1], 75) - np.percentile(sample[:, 1], 25)) / (len(sample[:, 1])) ** (1.0 / 3.0)
            )
            if bin_w == 0:
