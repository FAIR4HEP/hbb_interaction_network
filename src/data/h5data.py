...
class FilePreloader(Thread):
    ...
    Attributes:
      file_names (list): list of data files to use for training
        ...
        for cur_file_name in self.file_names:
            ...
            for cur_pos in range(0, num_in_file, self.batch_size):
                ...
                if next_pos <= num_in_file:
                    if self.spectators:
                        yield (
                            self.get_batch(cur_file_features, cur_pos, next_pos),
                    ...
                    if self.spectators:
                        leftovers = (
                            self.get_batch(cur_file_features, cur_pos, num_in_file),
