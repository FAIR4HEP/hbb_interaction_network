           ...
           for cur_pos in range(0, num_in_file, self.batch_size):
               ...
               if next_pos <= num_in_file:
                   ...
                   if self.spectators:
                       ...
                       leftovers = ( self.get_batch( cur_file_features, cur_pos, num_in_file ),
                                     self.get_batch( cur_file_labels, cur_pos, num_in_file ) )
    def my_generate_data(self,idx):
       """Yields batches of training data until none are left."""
       leftovers = None
       cur_file_name = self.file_names[idx]
       if self.spectators:
           cur_file_features, cur_file_labels, cur_file_spectators = self.load_data(cur_file_name)
       else:
           cur_file_features, cur_file_labels = self.load_data(cur_file_name)
       ...
       for cur_pos in range(0, num_in_file, self.batch_size):
           ...
           if next_pos <= num_in_file:
               if self.spectators:
                   return ( self.get_batch( cur_file_features, cur_pos, next_pos ),
                           self.get_batch( cur_file_labels, cur_pos, next_pos ),
                           self.get_batch( cur_file_spectators, cur_pos, next_pos ) )
               else:
                   return ( self.get_batch( cur_file_features, cur_pos, next_pos ),
...
class data_loader_shell(Dataset):
    def __init__(self, file_name, batch_size):
        super(data_loader_shell, self).__init__()
        self.H5 = H5Data(batch_size = batch_size,
                        cache = None,
                        preloading=0,
                        features_name='training_subgroup', 
                        labels_name='target_subgroup',
                        spectators_name='spectator_subgroup')
        self.len, self.file_len = self.H5.count_data()
#         print(self.len, self.file_len)
        self.batch_size = batch_size
        self.file_name = file_name
        self.H5.set_file_names(file_name)
#         self.gen = self.H5.generate_data()
        
    def __getitem__(self, idx):
        # if generate_data is used, set num_work to 1
        
        for i in range(len(file_len)):
            idx = idx - file_len[i]
            if idx <0:
                idx_ =i
        sub_X,sub_Y,sub_Z = self.H5.my_generate_data(idx_)
        return sub_X[2],sub_X[3],sub_Y,sub_Z #[:][0]#load_file(self.file_list[idx])
